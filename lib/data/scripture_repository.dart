...
class ScriptureRepository {
  ...
  final Map<int, List<ScriptureVerse>> _ggsCache = {};
  final Set<int> _loadedGgsChunks = {};
  ...
  Future<List<ScriptureVerse>> loadGgsAng(int ang) async {
    if (_ggsCache.containsKey(ang)) return _ggsCache[ang]!;
    final chunkIdx = ((ang - 1) ~/ 100).clamp(0, 14);
    if (!_loadedGgsChunks.contains(chunkIdx)) {
      await _loadGgsChunk(chunkIdx);
    }
    return _ggsCache[ang] ?? [];
  }

  Future<void> _loadGgsChunk(int idx) async {
    final label = idx.toString().padLeft(3, '0');
    final raw = await rootBundle.loadString('assets/data/scripture/ggs/ggs_$label.json');
    final data = jsonDecode(raw) as Map<String, dynamic>;
    data.forEach((angStr, lines) {
      final ang = int.parse(angStr);
      ...
      _ggsCache[ang] = verses;
    });
    _loadedGgsChunks.add(idx);
  }
